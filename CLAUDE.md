...
- `.github/workflows/ci.yml` : jobs `test` -> `build` -> `deploy-production` (ce dernier uniquement sur push vers `main`), plus `commitlint` en parallele (PR uniquement, voir section suivante). Les previews (branches `dev`, PR) restent gerees par l'integration Git native de Vercel, pas par ce workflow.
...
- Messages de commit au format [Conventional Commits](https://www.conventionalcommits.org/) (`type(scope): sujet`, voir `commitlint.config.js`), deja largement suivi dans l'historique avant d'etre rendu obligatoire. Verifie localement par le hook `commit-msg` de husky (`.husky/commit-msg`, installe automatiquement par `pnpm install` via le script `prepare`) et en CI sur chaque PR (job `commitlint`).
