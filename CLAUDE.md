...
- Versioning semver via `semantic-release` (voir `release.config.js` et le job `release` de `.github/workflows/ci.yml`) : chaque push sur `main` contenant des commits `feat`/`fix`/`perf` depuis le dernier tag declenche un tag git, une GitHub Release et une mise a jour de `front/public/CHANGELOG.md`, sans etape manuelle. Le type de commit determine le bump (`feat` -> minor, `fix`/`perf` -> patch, `BREAKING CHANGE` -> major), selon la convention `conventionalcommits` standard. Le job `release` tourne avant `deploy-production` (qui checkoute ensuite `main` a jour, apres le commit `chore(release): x.y.z [skip ci]` pousse par `@semantic-release/git`) pour que le changelog deploye soit toujours celui de la version deployee.
...
- Le changelog affiche au public (`/changelog`, lien en pied de page) est genere par `@semantic-release/release-notes-generator`, via l'enveloppe `scripts/semantic-release/generate-notes.cjs` qui : (1) ne garde que les commits `feat`/`fix`/`perf` (le reste - `chore`/`ci`/`docs`/`test`/`build`/`style`/`refactor` - n'a pas sa place dans un changelog oriente utilisateur), en excluant en plus les scopes techniques (`ci`, `e2e`, `test`, `deploy`, `dependabot`) ; (2) contourne un bug de resolution de `@semantic-release/release-notes-generator` avec `preset: "conventionalcommits"` qui peut faire tomber sur une version incompatible de `conventional-changelog-conventionalcommits` hoistee en fantome par pnpm (voir les commentaires du fichier pour le detail). `front/public/CHANGELOG.md` est commit (source de verite, plus jamais regenere a chaque deploiement) : `@semantic-release/git` y ajoute une nouvelle section a chaque release.
