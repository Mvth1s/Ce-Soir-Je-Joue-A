...
- Versioning semver via `semantic-release` (voir `release.config.js` et le job `release` de `.github/workflows/ci.yml`) : chaque push sur `main` contenant des commits `feat`/`fix`/`perf` depuis le dernier tag declenche un tag git et une GitHub Release, sans etape manuelle. Le type de commit determine le bump (`feat` -> minor, `fix`/`perf` -> patch, `BREAKING CHANGE` -> major), selon la convention `conventionalcommits` standard.
- **Les PR de promotion `dev` -> `main` doivent etre mergees avec "Merge" (`gh pr merge --merge`), jamais "Squash".** Un squash remplace tous les commits de `dev` par un seul commit `chore: promotion dev vers main` sur `main` : `semantic-release`, qui n'analyse que les commits reellement atteignables depuis `main`, perd alors toute trace des types `feat`/`fix`/`perf` individuels (le commit `chore` unique ne declenche aucun bump), donc plus aucune release ne se declenche automatiquement. Constate le 2026-08-07 : les PR #35/#40/#42/#44 avaient toutes ete squashees, rendant invisibles a `semantic-release` les commits `feat`/`fix` de #27/#33/#36/#41/#43 (reconstitue manuellement a posteriori via `gh release create` cible sur chaque commit d'origine, tags `v0.1.0` a `v1.0.0`). Les PR `feature -> dev`, elles, peuvent continuer a etre squashees normalement : chaque squash porte deja son propre type conventionnel honnete (titre de la PR), donc reste analysable une fois sur `main` via un vrai merge.
- Pas de plugin `@semantic-release/git` ni `@semantic-release/changelog` : `main` a un ruleset GitHub (PR obligatoire + statuts requis, voir `https://github.com/Mvth1s/Ce-Soir-Je-Joue-A/rules`) qui bloque tout `git push` direct sur cette branche, y compris depuis la CI avec `GITHUB_TOKEN` - ces deux plugins pousseraient un commit `chore(release)` directement dessus, donc echoueraient systematiquement (constate le 2026-08-07 : "GH013: Repository rule violations"). La GitHub Release creee par `@semantic-release/github` passe par l'API REST (pas un `git push` sur `refs/heads/main`), donc n'est pas concernee.
- Pas de plugin `@semantic-release/npm` : ce depot n'est pas un paquet publie (pas de consommateurs qui suivent une version dans `package.json`), le tag git et la GitHub Release suffisent comme source de verite de version. `package.json` garde une version fixe (`0.0.0`), non geree par semantic-release.
- Le changelog affiche au public (`/changelog`, lien en pied de page) est genere par `scripts/generate-changelog.ts` (`pnpm changelog`) a partir des GitHub Releases publiees (API REST, pas de git), dont le corps est deja au format attendu par `front/src/pages/ChangelogPage.vue` grace a `scripts/semantic-release/generate-notes.cjs` (l'enveloppe de `@semantic-release/release-notes-generator` utilisee par le job `release`) qui : (1) ne garde que les commits `feat`/`fix`/`perf` (le reste - `chore`/`ci`/`docs`/`test`/`build`/`style`/`refactor` - n'a pas sa place dans un changelog oriente utilisateur), en excluant en plus les scopes techniques (`ci`, `e2e`, `test`, `deploy`, `dependabot`) ; (2) contourne un bug de resolution de `@semantic-release/release-notes-generator` avec `preset: "conventionalcommits"` qui peut faire tomber sur une version incompatible de `conventional-changelog-conventionalcommits` hoistee en fantome par pnpm (voir les commentaires du fichier pour le detail). Ecrit `front/public/CHANGELOG.md`, jamais commit (regenere a chaque deploiement production, voir le job `deploy-production`).
- Pour previsualiser localement les notes de la prochaine release sans rien pousser : `pnpm release:dry-run` (`semantic-release --dry-run --no-ci`).
